...
describe('Contracts', () => {
  ...
  describe('Auction', () => {
    beforeEach(async () => {
      await contract.createAuction(name, description, price, {
        from: seller.address,
        value: toWei(0.02),
      })
      await contract
        .connect(reseller)
        .createAuction(name, description, price, {
          value: toWei(0.02),
        })
    })

    it('Should confirm Auction Price Change', async () => {
    ...
    it('Should confirm Auction Listing', async () => {
      result = await contract.getMyAuctions({ from: seller.address })
      expect(result).to.have.lengthOf(1)
    })

    // it('Should confirm Auction Purchase', async () => {
    //   result = await contract.getAuction(tokenId)
    ...
    it('Should confirm Price Change', async () => {
      result = await contract.getAuction(tokenId)
      expect(result.price).to.be.equal(price)
    ...
    // it('Should confirm Auction Relisting', async () => {
    //   // Status before offering
    //   result = await contract.getAuction(tokenId)
    //   expect(result.live).to.be.equal(false)
    //   // Seller offers on market
    //   await contract.offerAuction(tokenId, !biddable, secs, 0, 0, 0, {
    //     from: seller.address,
    //   })
    ...
    it('Should confirm Auction Bidding and Claim', async () => {
      result = await contract.getAuction(tokenId)
      expect(result.sold).to.be.equal(false)
      expect(result.price).to.be.equal(price)
